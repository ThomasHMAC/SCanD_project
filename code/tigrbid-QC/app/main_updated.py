...
from session_persistance import load_session_state
...
for _, row in current_batch.iterrows():
    ...
    svg_matches = glob(
        f"{fmriprep_derivative}/sub-{sub_id}/figures/sub-{sub_id}_*desc-reconall_T1w.svg"
    )
    ...
    for hemi, val in euler_vals.items():
        ...
        # ✅ Save back into session_state qc_records
        # st.session_state.qc_records.setdefault(sub_key, {})[hemi_key] = qc_choice
        st.radio(
            "",
            options=options,
            key=euler_key,
    ...
    seg_qc = st.radio(
        ...
        ("PASS", "FAIL", "UNCERTAIN"),
        key=f"{sub_id}_seg",
        # key=f"page_{st.session_state.current_page}_{sub_id}_seg",
        label_visibility="collapsed",
        index=None,
    )
